...
- Recovers Hardill-to-HA mappings from `friendlyDescription` / `additionalApplianceDetails`.
- Recognizes the exact duplicate-name format generated by v0.2.
- New Hardill devices embed their HA `entity_id` in `additionalApplianceDetails`, allowing stale Alexa appliance IDs to be resolved after a rename.

## v0.3.5 startup reliability

Remote Hardill device creation/update/deletion is no longer performed inside Home Assistant's config-entry setup. The bridge first restores appliance mappings from its local Store, starts MQTT, and reconciles the remote device inventory after Home Assistant has completed startup. This prevents slow sequential Hardill web requests from causing Home Assistant's bootstrap timeout to cancel the integration setup.
